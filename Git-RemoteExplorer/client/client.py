...
def send_data(data, addr, file=False):
    ...
    client_socket.connect((addr,int(ui.port.text())))  # 替换 'server_ip' 为服务端的IP地址
...
class Ui_MainWindow(QtWidgets.QWidget):
    ...
    def download(self):
        ...
        if path:
            # print(send_data(f"command:{self.username.text()},{self.password.text()},upload,{self.address.text().strip('/')}",self.server_addr.text(),file=True))
    ...
    def save_pickle (self):
        with open('quick.pkl','wb') as f:
            pk.dump(self.quickls,f)
    def read_pickle (self):
        with open('quick.pkl','rb') as f:
            self.quickls = pk.load(f)
        pass
...
# if __name__ == '__main__':
#     send_data('command:Admin,114514810,ls,C:/')